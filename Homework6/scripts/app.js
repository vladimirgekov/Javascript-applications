...
(() => {
  ...
  const app = Sammy("#main", function () {
    ...
    this.post("#/login", function (ctx) {
      ...
      postRequests(
        ctx,
        ["user", "login", "Basic", { username, password }],
        "#/home"
      );
    });
    this.post("#/create", function (ctx) {
      ...
      postRequests(
        ctx,
        ["appdata", "teams", "Kinvey", { name, comment, members }],
        "#/catalog"
      );
    });
  ...
  function loadPage(ctx, path) {
    getUserInfo(ctx);
    ctx.loadPartials(templatesPaths).then(function () {
      this.partial(path);
    });
  }
  ...
  function joinTeam(ctx, data, id) {
    ...
    putRequest(
      ctx,
      ["appdata", `teams/${id}`, "Kinvey", data],
      `#/catalog/${id}`
    );
  }
  function leaveTeam(ctx, data, id) {
    const index = data.members.findIndex(
      (obj) => obj.username === sessionStorage.username
    );
