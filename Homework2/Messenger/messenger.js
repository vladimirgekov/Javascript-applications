function attachEvents() {
  ...
  function loadMessages() {
    $.get(baseServiceUrl + ".json").then(displayMessages);
  }
  ...
  function displayMessages(messages) {
    ...
    for (let message of Object.keys(orderedMessages)) {
      $("#messages").append(
        `${orderedMessages[message]["author"]}: ${orderedMessages[message]["content"]}\n`
      );
    }
  }
  ...
  function createMessage() {
    ...
    $.post(baseServiceUrl + ".json", JSON.stringify(data)).then(loadMessages);
  }
